#' @name 03_USGS_7Q2Regression
#' 
#' @author Sara Bangen
#'
#' @param in.data Path to Baseflow_7Q2_Regression_Variables.csv
#' @param out.f Output folder path


# Set user defined arguments:
in.data = "C:/.../LowFlow_RegressionAnalysis_7Q2/Baseflow_7Q2_Regression_Variables.csv"
...
create.model.df = function(x){  
    tribble(
      ...
      "da.sq.mi","lm", list(formula = as.formula("q.cfs ~ da.sq.mi"),data = x),    
      "da.sq.mi + elev.ft.8","lm", list(formula = as.formula("q.cfs ~ da.sq.mi + elev.ft.8"),data = x),
      "da.sq.mi + relief.ft.8","lm", list(formula = as.formula("q.cfs ~ da.sq.mi + relief.ft.8"),data = x),
      "da.sq.mi + slope.pct.8","lm", list(formula = as.formula("q.cfs ~ da.sq.mi + slope.pct.8"),data = x),
...
# Function that reshapes the results from apply.models
reshape.results = function(x){   
    x %>% 
    select(model.name, model.fit) %>% 
    spread(model.name, model.fit)
  }
...
nfd.region = run.regression(by.nfd, 'Baseflow_7Q2_NFDRegions', 'NFDName', 'NFDCode')
dwr.region = run.regression(by.dwr, 'Baseflow_7Q2_DWRRegions', 'DWRName', 'DWRCode')
